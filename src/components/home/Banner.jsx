  ...
  return (
    <section className="relative w-full pb-24 lg:h-screen overflow-hidden bg-night flex flex-col">
      ...
      <div className="absolute inset-0 bg-hero-gradient z-0">
        ...
        <div className="absolute inset-0 z-10 pointer-events-none">
          {fallingStarsData.map((star) => (
            <motion.span
              key={star.id}
              initial={{ top: "-5%", left: star.left, opacity: 0, scale: 1 }}
              animate={{
                top: ["0%", "100%"],
                x: [0, -150],
                opacity: [0, 1, 1, 0],
        ...
        {/* Moon Glow */}
        <div className="absolute top-24 right-6 sm:top-28 sm:right-10 md:top-32 md:right-20 lg:top-20 lg:right-20 w-8 h-8 sm:w-12 sm:h-12 md:w-16 md:h-16 rounded-full moon-glow opacity-80 z-10" />
      </div>
      ...
      <div className="relative z-20 container mx-auto px-4 sm:px-6 pt-24 pb-6 sm:pt-28 sm:pb-8 lg:pt-0 lg:pb-0 lg:flex-1 lg:flex lg:items-center">
        ...
        >
          {/* Left: Image Showcase (desktop only) */}
          <motion.div
            variants={buildingVariants}
            className="relative hidden lg:block order-last lg:order-first"
          >
            <div className="relative group overflow-hidden rounded-[2rem] h-[calc(100vh-220px)] min-h-[500px] max-h-[720px] shadow-2xl border-2 border-white/10 bg-white/5 backdrop-blur-sm transition-all duration-300 hover:border-luxe-gold/40">
              <Image
          ...
          {/* Right: Text & CTA */}
          <div className="space-y-3.5 sm:space-y-5 lg:space-y-6 lg:pl-6 flex flex-col justify-center text-center lg:text-left">
            <motion.div
              variants={itemVariants}
              className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/10 border border-white/20 backdrop-blur-md shadow-sm mx-auto lg:mx-0 max-w-fit"
            >
              <span className="text-amber-300 font-bold text-[9px] sm:text-xs uppercase tracking-widest">
            ...
            <motion.h1
              variants={itemVariants}
              className="font-serif text-3xl sm:text-4xl md:text-6xl lg:text-7xl font-extrabold text-white leading-[1.15] tracking-tight"
            >
              Discover Luxury <br />
            ...
            <motion.p
              variants={itemVariants}
              className="text-xs sm:text-base md:text-lg text-zinc-300 max-w-sm sm:max-w-xl mx-auto lg:mx-0 leading-relaxed"
            >
            ...
            <motion.div
              variants={itemVariants}
              className="flex flex-col sm:flex-row items-center gap-3 sm:gap-6 justify-center lg:justify-start pt-1 sm:pt-2"
            >
              <Link href="/properties" className="w-full sm:w-auto">
                <button className="group w-full sm:w-auto bg-brand hover:bg-brand-dark text-white px-6 py-2.5 sm:px-7 sm:py-3 rounded-full font-bold text-xs sm:text-sm transition-all duration-300 shadow-xl hover:shadow-2xl active:scale-95 flex items-center justify-center gap-2.5 border border-white/10">
